  ...
  return (
    <div className='w-full max-w-96 bg-slate-800 p-5 rounded-xl'>
      ...
      <div className='flex flex-col gap-1 mt-5'>
        <div>Nama</div>
        <input id='nama' placeholder='nama' className='rounded-full px-2 text-slate-800 py-1' onChange={e => setNama(e.target.value)} autoFocus />
      ...
      <div className='flex gap-2 justify-end mt-5'>
        <button className='px-2 py-1 w-20 rounded-full bg-green-600' onClick={() => postMumi()}>Save</button>
        <button className='px-2 py-1 w-20 rounded-full bg-red-500' onClick={() => onClose()}>Cancel</button>
      </div>
    </div>
    )
}
